...
import { selectClaudeConfigDetected, selectClaudeConfigSource, selectClaudeProvider, detectClaudeConfig } from '../../../store/slices/llmInstallationSlice';
...
function LlmInstallationStep({ onNext, onSkip, onNavigateToOnboarding, versionId }: LlmInstallationStepProps) {
  ...
  // Detect Claude configuration on component mount
  useEffect(() => {
    const detectConfig = async () => {
      setIsDetecting(true);
      try {
        await dispatch(detectClaudeConfig() as any);
      } catch (err) {
        console.error('Failed to detect Claude config:', err);
      } finally {
    ...
    <div className="space-y-6">
      ...
      {isDetecting ? (
        <div className="text-center space-y-4 py-8">
          ...
          <p className="text-lg font-medium">Detecting Claude CLI configuration...</p>
        </div>
      ) : !claudeConfigDetected && !isClaudeValid ? (
        <div className="space-y-4">
          <div className="flex items-center justify-center gap-2 text-yellow-600 dark:text-yellow-500 bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
            <AlertCircle className="h-5 w-5" />
            <span>Claude CLI is not configured</span>
          </div>
          <div className="text-sm text-muted-foreground text-center">
            <p>To use LLM-powered installation, you need to configure Claude Code CLI first.</p>
          </div>
          {onNavigateToOnboarding && (
            <Button
              onClick={handleConfigureClaude}
              className="w-full px-4 py-3 bg-primary text-primary-foreground rounded-lg flex items-center justify-center gap-2"
            >
          ...
          <div className="flex items-center justify-between p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <span>Claude CLI configured</span>
                {claudeProvider && (
                  <span className="text-sm text-muted-foreground">
                    Provider: {getProviderDisplayName(claudeProvider)}
                  </span>
                )}
                {claudeConfigSource && (
                  <span className="text-xs text-muted-foreground">
                    Source: {getSourceDisplayName(claudeConfigSource)}
                  </span>
                )}
              </div>
            </div>
